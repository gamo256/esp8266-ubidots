...
float Ubidots::getValue(char* id){
    ...
    _client.println(F("AT+CIPMODE=0"));
    if(strstr(readData(1000),"OK")==NULL){
        Serial.println(F("Error"));        
        return false;
    }
    _client.println(F("AT+CIPSEND=185"));
...
bool Ubidots::sendAll(){
    ...
    for(i=0; i<currentValue;){
        ...
        all += str;
        all += "\"";
        if(String((val+i)->context_1)!="" && String((val+i)->context_2)!=""){
           all += ", \"context\":{";
...
bool Ubidots::saveValue(char *id, float value){
    int i;
    String str;
    ...
    _client.print(F("POST /api/v1.6/variables/"));
    _client.print(id);
    _client.println(F("/values HTTP/1.1"));
    _client.println(F("Host: things.ubidots.com"));
    _client.println(F("User-Agent: Arduino-ESP8266/1.0"));
    _client.print(F("X-Auth-Token: "));
    _client.println(_token);
